...
export default function GuruTab({
  ...
  // Edit Guru details state
  const [editingGuru, setEditingGuru] = useState<Guru | null>(null);

  const handleFormSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (editingGuru) {
      if (!editingGuru.nama || !editingGuru.nip) {
        alert('Nama dan NIP wajib diisi.');
        return;
      }
      if (onUpdateGuru) {
        onUpdateGuru(editingGuru);
      }
      setEditingGuru(null);
    } else {
      handleAddGuru(e);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 space-y-4 shadow-xs">
          <h3 className="font-semibold text-sm text-slate-800 border-b border-slate-100 pb-2 flex items-center gap-1.5">
            {editingGuru ? (
              <>
                <Pencil className="w-4 h-4 text-indigo-600" /> Edit Biodata Guru
              </>
            ) : (
              <>
          ...
          <form onSubmit={handleFormSubmit} className="space-y-3.5 text-xs">
            <div>
              ...
              <input 
                type="text" 
                value={editingGuru ? editingGuru.nama : (newGuru.nama || '')}
                onChange={(e) => {
                  if (editingGuru) {
            ...
            <div>
              ...
              <input 
                type="text" 
                value={editingGuru ? editingGuru.nip : (newGuru.nip || '')}
                onChange={(e) => {
                  if (editingGuru) {
                    setEditingGuru({...editingGuru, nip: e.target.value});
                  } else {
                    setNewGuru({...newGuru, nip: e.target.value});
                  }
                }}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select 
                  value={editingGuru ? editingGuru.jenis_kelamin : (newGuru.jenis_kelamin || 'Laki-laki')}
                  onChange={(e) => {
                    const value = e.target.value as 'Laki-laki' | 'Perempuan';
                    if (editingGuru) {
                      setEditingGuru({...editingGuru, jenis_kelamin: value});
                    } else {
                      setNewGuru({...newGuru, jenis_kelamin: value});
                    }
                  }}
              ...
              <div>
                ...
                <input 
                  type="text" 
                  value={editingGuru ? editingGuru.no_hp : (newGuru.no_hp || '')}
                  onChange={(e) => {
                    if (editingGuru) {
                      setEditingGuru({...editingGuru, no_hp: e.target.value});
                    } else {
                      setNewGuru({...newGuru, no_hp: e.target.value});
                    }
                  }}
                  placeholder="0812..."
                  className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-slate-800 placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/20 transition-all font-medium"
            ...
            <div className="flex items-center gap-2 pt-2">
              <input 
                type="checkbox" 
                id="status_aktif"
                checked={editingGuru ? editingGuru.status_aktif : (newGuru.status_aktif !== undefined ? newGuru.status_aktif : true)}
                onChange={(e) => {
                  if (editingGuru) {
                    setEditingGuru({...editingGuru, status_aktif: e.target.checked});
                  } else {
                    setNewGuru({...newGuru, status_aktif: e.target.checked});
                  }
                }}
            ...
            <div className="flex gap-2 pt-2">
              {editingGuru && (
                <button 
                  ...
                  className="w-1/3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-lg transition cursor-pointer"
                >
                  Batal
                </button>
              )}
              <button 
                type="submit" 
                className={`py-2 text-white font-bold rounded-lg transition cursor-pointer ${editingGuru ? 'w-2/3 bg-indigo-600 hover:bg-indigo-700' : 'w-full bg-indigo-600 hover:bg-indigo-700'}`}
              >
                {editingGuru ? 'Simpan Guru' : 'Daftarkan Guru'}
              </button>
            </div>
          </form>
        </div>
                  ...
                  return (
                    <tr key={g.id} className="hover:bg-slate-50/55 group">
                      ...
                      <td className="py-3 px-3">
                        ...
                        >
                          <Sliders className="w-3 h-3 text-indigo-600" /> {hasCustomPref ? 'Ada Aturan' : 'Atur Pref'}
                        </button>
                      </td>
                      <td className="py-3 px-3">
                        <div className="flex items-center justify-end gap-1">
                          <button 
                            ...
                            className={`p-1.5 rounded transition cursor-pointer ${editingGuru?.id === g.id ? 'bg-indigo-100 text-indigo-700 border border-indigo-200' : 'text-slate-500 hover:text-indigo-600 hover:bg-slate-105'}`}
